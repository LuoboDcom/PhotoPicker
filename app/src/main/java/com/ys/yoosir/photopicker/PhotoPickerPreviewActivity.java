...
import com.ys.yoosir.photopicker.photoview.PhotoViewAttacher;
import com.ys.yoosir.photopicker.util.MQBrowserPhotoViewAttacher;
import com.ys.yoosir.photopicker.widget.MQHackyViewPager;
 ...
 * 描述:图片选择预览界面
 */
public class PhotoPickerPreviewActivity extends Activity implements View.OnClickListener,
            ...
            if (mSelectedImages.contains(currentImage)) {
                ...
                if (mMaxChooseCount == 1) {
                    ...
                    if (mMaxChooseCount == mSelectedImages.size()) {
                        Toast.makeText(this,getString(R.string.toast_photo_picker_max,mMaxChooseCount),Toast.LENGTH_SHORT).show();
                    } else {
                        mSelectedImages.add(currentImage);
                        mChooseTv.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.ic_cb_checked, 0, 0, 0);
                        renderTopRightBtn();
    ...
    private class ImagePageAdapter extends PagerAdapter {
        ...
        public View instantiateItem(ViewGroup container, int position) {
            final YSImageView imageView = new YSImageView(container.getContext());
            container.addView(imageView, ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
            final MQBrowserPhotoViewAttacher photoViewAttacher = new MQBrowserPhotoViewAttacher(imageView);
            photoViewAttacher.setOnViewTapListener(PhotoPickerPreviewActivity.this);

            imageView.setDrawableChangedCallback(new YSImageView.OnDrawableChangedCallback() {
                @Override
                public void onDrawableChanged(Drawable drawable) {
                    if (drawable != null && drawable.getIntrinsicHeight() > drawable.getIntrinsicWidth() && drawable.getIntrinsicHeight() > screenHeight) {
            ...
            MQImage.displayImage(PhotoPickerPreviewActivity.this, imageView, mPreviewImages.get(position), R.mipmap.ic_holder_dark, R.mipmap.ic_holder_dark, screenWidth, screenHeight, null);
